...
```{r}
x = housing$OverallCond
housing$quality_cond = ifelse(x <= 5, "bad", ifelse(x==6, "Above Average", ifelse(x==7, "Good", ifelse(x==8, "Very Good", ifelse(x==9, "Excellent", "Very Excellent"))))) 

ggplot(housing) + 
  geom_boxplot(aes(quality_cond, SalePrice))

housing$quality_cond2 = ifelse(x>5, "good", "bad")
t.test(SalePrice ~ quality_cond2, data = housing)
...
```{r}
housing$year_cond = ifelse(housing$YearBuilt<1970, "old", "new")

ggplot(housing) + 
  geom_boxplot(aes(year_cond, SalePrice))

t.test(SalePrice ~ year_cond, data = housing)
```
...
ggplot(housing2) + 
  geom_boxplot(aes(year1970, SalePrice))
...
```{r}
ggplot(housing) + 
  geom_boxplot(aes(CentralAir, SalePrice))

t.test(SalePrice ~ CeltralAir, data = housing)
...
```{r}
housing$garage_cond = ifelse(!is.na(housing$GarageFinish), "yes", "no")

ggplot(housing) + 
  geom_boxplot(aes(garage_cond, SalePrice))

t.test(SalePrice ~ garage_cond, data = housing)
...
```{r}
housing3 = housing %>% filter(GarageCars>0) %>% mutate(gara_2 = ifelse(GarageCars<2, "<2", ">2"))
ggplot(housing3) + 
  geom_boxplot(aes(gara_2, SalePrice))

t.test(SalePrice ~ gara_2, data = housing3)
...
```{r}
housing4 = housing %>% filter(Neighborhood %in% c("OldTown", "Edwards"))

ggplot(housing4) + 
  geom_boxplot(aes(Neighborhood, SalePrice))

t.test(SalePrice ~ Neighborhood, data = housing4)
...
```{r}
#housing = read_csv("~/shared/minor2_2018/1-intro/data/housing.csv")
#housing = housing %>% select(SalePrice, OverallCond, Neighborhood, Utilities, LotArea, YearBuilt, CentralAir, GarageFinish, GarageCars, PoolQC, HeatingQC)
